...
### 什么是爬虫
说到爬虫可能有比很多人会觉得，它很神秘，技术很难掌握。如果说是比较复杂的爬虫网络系统或者是聚焦爬虫这种比较高级的，大量依赖算法进行网页分析筛选过滤，然后根据一定的搜索策略从队列中选择URL进行爬取的，可能是会比较难的。但是对于比较传统爬虫，定向的URL抓取网页，就比较简单了，其实就是**发起一个或者若干个网络请**求，然后拿到请求结果（响应体，可以是网页、数据包、文件等等），根据我们的需求**处理请求结果**，拿到我们想要的数据，然后把**数据持久化**，存储到数据库或者文件里面等等，有必要的话还可以对数据做一些整理和可视化等，这个就涉及到另一块领域了——数据分析。
 ...
 **动态：** 有的网站就很皮了，它网站上的一部分数据可能是动态渲染的，就是通过js ajax请求数据，动态渲染到页面上。  
 ...
 ### 爬虫效率  
 如何设计一个高效的爬虫系统，怎样的并发策略，怎么控制内存释放，或者说设计一个分布式爬虫等等  
 本人技术有限目前未研究这么深，如果以后有机会，可以填一填这方面的坑。
